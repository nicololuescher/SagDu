...
VALUES
  -- grains / starch
  (1,  'oatmeal',           68.000,  2.400, 12.000, 1.400, 1.700, TRUE, TRUE,  TRUE,  TRUE,  TRUE),
  ...
  -- fruit / veg
  (2,  'banana',            89.000,  1.100, 23.000, 0.300, 2.600, TRUE, TRUE,  TRUE,  TRUE,  TRUE),
  (4,  'spinach',           23.000,  2.900,  3.600, 0.400, 2.200, TRUE, TRUE,  TRUE,  TRUE,  TRUE),
  (9,  'broccoli',          34.000,  2.800,  7.000, 0.400, 2.600, TRUE, TRUE,  TRUE,  TRUE,  TRUE),
  ...
  -- dairy / eggs
  (3,  'milk, 2%',          42.000,  3.400,  5.000, 1.000, 0.000, TRUE, FALSE, TRUE,  FALSE, TRUE),
  (11, 'yogurt, Greek 0%',  59.000, 10.000,  3.600, 0.400, 0.000, TRUE, FALSE, TRUE,  FALSE, TRUE),
  (8,  'egg',               78.000,  6.300,  0.600, 5.300, 0.000, TRUE, FALSE, TRUE,  TRUE,  TRUE)
ON CONFLICT (id) DO NOTHING;

---------------------------------------------------------------------
-- User inventory (3 items)
---------------------------------------------------------------------
INSERT INTO user_ingredient (user_id, ingredient_id, quantity) VALUES
  (1, 1, 500.000),   -- oatmeal (g)
  (1, 2,   5.000),   -- bananas (pieces)
  (1, 3, 1000.000)   -- milk (ml)
...
SELECT * FROM (
  SELECT (SELECT meal_id FROM b1), 1,  40.000 UNION ALL
  SELECT (SELECT meal_id FROM b1), 3, 200.000 UNION ALL
  SELECT (SELECT meal_id FROM b1), 2,   1.000 UNION ALL

  SELECT (SELECT meal_id FROM b2), 11, 200.000 UNION ALL
  SELECT (SELECT meal_id FROM b2), 1,  30.000 UNION ALL
  SELECT (SELECT meal_id FROM b2), 2,   1.000 UNION ALL

  SELECT (SELECT meal_id FROM b3), 8,   2.000 UNION ALL
  SELECT (SELECT meal_id FROM b3), 4,  80.000 UNION ALL
  SELECT (SELECT meal_id FROM b3), 13,100.000
...
SELECT * FROM (
  ...
  SELECT (SELECT meal_id FROM d3), 2,   1.000
) s(meal_id, ingredient_id, quantity)
ON CONFLICT (meal_id, ingredient_id) DO UPDATE SET quantity = EXCLUDED.quantity;