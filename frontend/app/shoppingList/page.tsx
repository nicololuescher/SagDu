...
export default function ShoppingList() {
    ...
    const handleRemove = (id: number) => {
        toast("Item was added to Inventory", {
          action: {
            label: "Undo",
            onClick: () => console.log("Undo"), //Doesn't actually do anything but looks nice
          },
        })
                            ...
                            <Card className="w-full">
                                <CardHeader className="flex flex-row items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {item.diet === "vegan" && <Badge variant="outline">🥦 Vegan</Badge>}
                                        {item.diet === "vegetarian" && <Badge variant="outline">🧀 Vegetarian</Badge>}
                                        {item.diet === "none" && <Badge variant="destructive">🍖 Meat</Badge>}
                                    </div>
